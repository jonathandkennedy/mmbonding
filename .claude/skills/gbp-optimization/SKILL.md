...
> **2026 shift — prominence → popularity.** Google's local weighting has moved toward popularity/engagement signals (CTR, calls, direction requests, review velocity, post/photo activity) and away from static prominence (raw link/review counts, business tenure). Active profiles now outrank established-but-inactive ones, and being open when the user searches is roughly a top-5 factor. The same engagement signals feed AI-generated local answers. Frame optimization around demonstrating ongoing activity, not one-time setup.
...
8. **Linked Social** (2026 — read carefully): Google indexes content from *linked, active* social accounts and can surface their offers/events on the profile. But social *signals* (likes/shares) are NOT a confirmed direct ranking factor — Google has said so repeatedly. Treat linked + active social as an engagement/indexing play, not a ranking lever
...
**Phone**: Local number (not toll-free) as primary. Same number across all citations. (Google Business chat/messaging is being retired — new conversations end July 15, 2026 and chat fully ends July 31, 2026, with WhatsApp as the replacement. Don't rely on GBP messaging; keep call and website CTAs strong.)
...
**Guidelines**: 10-15 minimum, add 2-3/month. Descriptive filenames. No stock photos. 720px+ wide. JPG or PNG. (Don't bother geo-tagging — Google strips EXIF on upload and geotagged photos have no ranking effect.)
...
Answer everything (owner answers show first). Monitor weekly for competitor/spam Q&A. Answer in natural language. **2026 note:** Google is replacing user-facing Maps Q&A with an AI-generated Q&A (answers drawn from your profile, reviews, and website, gated behind owner review). Seeding your own Q&A matters less now — what matters is that the underlying profile/website data is complete and accurate so AI answers are correct, and that you review AI-suggested answers before they publish.
...
| Want to optimize the pack snippet (CTR) | Influence justifications once you rank | `gbp-justifications` |
| Engagement/CTR underperforming or under attack | Optimize genuine engagement; defend against manipulation | `behavioral-engagement` |
| A competitor is outranking you via spam | Audit and report the violations | `gbp-spam-enforcement` |
| Rank near the pin but not at distance | Work proximity legitimately | `proximity-location-strategy` |
