...
- **Bulk posts**: Template posts with location variables. **2026:** Google now offers *native* multi-location post scheduling (compose once, publish to many locations), reducing reliance on third-party schedulers. The GBP-in-Gemini drafting assistant is *not* available to anyone managing more than one verified profile, so it doesn't fit agency/multi-location workflows.
