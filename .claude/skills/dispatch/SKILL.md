...
**Before asserting any ranking factor as fact, load `evidence-standards`** — it assigns a confidence tier so opinion and folklore don't get presented as proven.
...
### "What should I post on Google?" / "GBP posts" / "post from local news"
1. `gbp-posts` — post types, cadence, templates
2. `local-news-posts` — when posts should be built from local news or topical/seasonal trends (includes a compliance gate for legal/medical/financial verticals)
3. `localseodata-tool` — `keyword_opportunities` to confirm a topic is rising locally
...
### "Is that proven?" / "How do you know X is a ranking factor?"
1. `evidence-standards` — assign a confidence tier before stating any ranking claim

### "Does X actually work?" / "Can we test this?" / "Prove it moves rankings"
1. `local-seo-experiments` — design a controlled test
2. `geogrid-analysis` — baseline + measurement

### "A competitor is spamming" / "fake listings outranking me" / "we're being review-bombed"
1. `gbp-spam-enforcement` — audit, report, and defend
2. `local-competitor-analysis` — confirm it's spam vs. legitimate competition
3. If your own listing got suspended: `gbp-suspension-recovery`

### "Optimize my local pack snippets" / "why does that show under their listing"
1. `gbp-justifications` — influence the justification snippets
2. `local-landing-pages` — the on-page content that feeds website-mention justifications

### "Improve my clicks/engagement" / "CTR" / "are CTR bots worth it"
1. `behavioral-engagement` — legitimate engagement optimization + attack defense (no manipulation)

### "Build my brand/entity authority" / "knowledge panel" / "show up in AI"
1. `entity-authority` — entity consistency, `sameAs`, branded demand
2. `ai-local-search` — AI visibility tactics on top

### "I rank near my office but not farther out" / "where should I open" / "proximity"
1. `proximity-location-strategy` — legitimate proximity levers and the line not to cross
2. `geogrid-analysis` — measure the falloff curve
